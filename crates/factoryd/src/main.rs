...
async fn run_serve(args: DaemonArgs) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let daemon = LocalDaemon::bind(config, &store)
        ...
        // CAS keeps the compressed transcript as the evidence projection;
        // human-readable copies are a disposable `make status` presentation.
...
mod tests {
    ...
    fn init_arguments() -> Vec<String> {
        vec![
            ...
            "/opt/factory/bin/factory-tea-host".to_owned(),
            "--host-source-root".to_owned(),
            "/opt/factory-source/crates/factory-tea-host".to_owned(),
