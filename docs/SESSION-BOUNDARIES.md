...
| Live provider spend | Provider accounting is refreshed when each provider turn settles. The host checks the explicit accounting callback at the core's `ModelTurnUsage`/`TurnEnd` lifecycle boundary; a reported total at or above the packet's remaining campaign allowance cancels the core run and emits `cost_limit`. In-flight provider spend is unknowable until that response returns. | `crates/factory-pi-host/src/execution.rs`, `crates/factory-pi-host/src/main.rs`, `/Users/josh/d/pi-agent-core-rs/crates/pi-agent-core/src/event.rs` |
