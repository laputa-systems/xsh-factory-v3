...
| Provider output-token request | Factory does not send the application `output_token_limit` as `max_tokens`. The OpenRouter adapter's default is no request cap; an embedding may still opt into an explicit provider hint with `with_max_tokens`. Provider/model-native capability limits remain external facts. | `/Users/josh/d/tea/crates/tea-providers/src/openrouter/config.rs`, `crates/factory-tea-host/src/main.rs` |
